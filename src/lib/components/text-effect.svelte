<script lang="ts">
  ...
  const transform = (node: HTMLDivElement) => {
    ...
    node.innerHTML =
      node.textContent?.replace(/\S/g, "<span class='letter' style='display: inline-block'>$&</span>") ?? '';
    initializeTimeline();
    return {
      destroy() {
        timeline?.pause();
  ...
  $: if (timeline)
    timeline.seek(clamp(mapLinear(progress, _in.start, _in.end, 0, completeDuration), 0, completeDuration));
